...
void nodeDisplay(raaNode *pNode) // function to render a node (called from display())
{
	...
	{
		float afCol[] = { 0.0f, 1.0f, 1.0f, 1.0f };
		utilitiesColourToMat(afCol, 2.0f);
	}
	else if (continent == 6)
	{
	...
	{
		float afCol[] = { 1.0f, 0.5f, 0.0f, 1.0f };
		utilitiesColourToMat(afCol, 2.0f);
	}
	else
	{
		float afCol[] = { 1.0f, 1.0f, 1.0f, 1.0f };
		utilitiesColourToMat(afCol, 2.0f);
	}
	...
	else if (world_system == 2)
	{
		glutSolidCube(15);
	}
	else if (world_system == 3)
	{
		glutSolidCone(15, 20, 50, 500);
	}
